...
class MATA030(unittest.TestCase):
	...
	def test_MATA030_CT133(self):
 	...
 	def test_MATA030_CT133_1(self):
		...
		self.oHelper.SetButton('Incluir')
		self.oHelper.SearchBrowse(f'D MG    {cliente+loja}', 'Filial+codigo + Loja')
		self.oHelper.SearchBrowse('D MG    000023', 'Filial+codigo + Loja')
		
  		self.oHelper.SearchBrowse('D MG    000023', key=5, index=True)
  
  		self.oHelper.SetBranch('D MG 01')
		self.oHelper.SetValue('Cliente',cliente)
  		self.oHelper.SetValue('A1_COD',cliente)
  		self.oHelper.SetValue('A1_PRODUTO',produto, grid=true)
  		self.oHelper.SetButton('Cancelar')
		self.oHelper.SetButton('Outras Ações','Alterar')
		self.oHelper.SetButton('Outras Ações','Outros',"Importar")
